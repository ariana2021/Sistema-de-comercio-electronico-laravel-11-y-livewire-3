                                                    ...
                                                    <div class="profile__main-inner d-flex flex-wrap align-items-center">
                                                        <div class="profile__main-thumb">
                                                            ...
                                                            <img id="profile-preview"
                                                                src="{{ auth()->user()->avatar ? Storage::url(auth()->user()->avatar) : asset('assets/principal/img/users/user-10.jpg') }}"
                                                                alt="Foto de perfil">
                                                ...
                                                <div class="col-md-4 col-sm-6">
                                                    <div class="profile__main-info-item">
                                                        <div class="profile__main-info-icon">
                                                            <span>
                                                                <span
                                                                    class="profile-icon-count">{{ number_format($availableCashback, 2) }}</span>
                                                                <svg viewBox="0 0 512 512">
                                                                    <path
                                                        ...
                                                        <h4 class="profile__main-info-title">CashBack</h4>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
...
@push('scripts')
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            document.getElementById('profile-thumb-input').addEventListener('change', function(event) {
                let file = event.target.files[0];

                if (file) {
                    let reader = new FileReader();

                    // Mostrar preview antes de subir la imagen
                    reader.onload = function(e) {
                        document.getElementById('profile-login').src = e.target.result;
                    };
                    ...
                    fetch("{{ route('profile.update-photo') }}", {
                        ...
                        .then(data => {
                            if (data.success) {
                                alertSweet('Perfil Actualizado', 'success');
                                document.querySelector('.profile__main-thumb img').src = data.image_url;
                            } else {
                                alertSweet('Error al subir la imagen.', 'error');
                            }
                        })
